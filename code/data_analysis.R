...
# INSTRUCTIONS
#
# supporting files located in 'clean_data'
#
#The code must be run sequentially downwards.
#Results will be stored in a new 'results' folder within this wd.
#
...

{
  ...
  #save, if desired
  ggsave("./results/convergence_subprime_prime_demo_COVID.pdf",height=4,width=6,device = cairo_pdf)
  rm(list=ls())
}
...

{
  ...
  write.csv(master, "./results/bal_by_age2.csv")
  ...
  ggplot(data = master, aes(x=month, y=ltv)) +
    ...
    theme(axis.title.x=element_text(size=9, family="Times New Roman"),
          ...
          legend.title=element_text(size=10, family="Times New Roman"))# +
  #save, if desired
  ggsave("./results/ltv_by_age.pdf",height=4,width=6,device = cairo_pdf)
  rm(list=ls())
}

######################################################################################
######################################################################################
# Table 2: Estimated Savings by Risk Band, Loan Age
######################################################################################
######################################################################################
...
#balances for table 3; see `refi_svgs2.xlsx` for table calcs
{
  ...
  master = read.csv("./results/bal_by_age2.csv")
...
#figure plot
{
  ...
  ggsave("./results/repay_2017.pdf",height=4,width=6,device=cairo_pdf)
...
#figure plot
{
  ...
  #save plot, if desired
  ggsave("./results/convergence_subprime_prime_demo_COVID2019.pdf",height=4,width=6,device = cairo_pdf)
  rm(list=ls())
}
...

{
  ...
  ggplot(data=test_dat,
    ...
    facet_grid(rows = vars(low_cat_f), cols = vars(high_cat_f))
  #save, if desired
  ggsave("./results/haz_grid_default2019.pdf",height=5,width=5,device = cairo_pdf)
  rm(list=ls())
}
...
#figure plot
{
  ...
  #annotate("label", x = 23, y = 0.0275, label = "Approx. Mar 2020 + 3 mo.", family="Times New Roman")
  #save, if desired
  ggsave("./results/convergence_subprime_prime_demo_new2017.pdf",height=4,width=6,device = cairo_pdf)
  rm(list=ls())
}
...

{
  ...
  ggplot(data=plot_df,
    ...
    theme(axis.title.x=element_text(size=9, family="Times New Roman"),
          ...
          legend.title=element_text(size=10, family="Times New Roman"))
  #save, if desired
  ggsave("./results/haz_def_new_all.pdf",height=4,width=6,device = cairo_pdf)
  rm(list=ls())
}
...
{
  dat <- read.csv('./clean_data/csh_lam.csv')
  ...
  #save, if desired
  ggsave("./results/sim_study.pdf",height=4,width=6,device = cairo_pdf)
  rm(list=ls())
}